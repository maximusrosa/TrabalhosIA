...
start = time.time()
hamming_sol = astar_hamming(estado_inicial)
end = time.time()
tempo_exec = (end - start) * 1000
print("A* com Hamming. Tamanho: ", len(hamming_sol), ", Tempo: {:.4f} ms".format(tempo_exec))
print("Solução: \n", hamming_sol, '\n')

start = time.time()
manhatan_sol = astar_manhattan(estado_inicial) 
end = time.time()
tempo_exec = (end - start) * 1000
print("A* com Manhattan. Tamanho: ", len(manhatan_sol), ", Tempo: {:.4f} ms".format(tempo_exec))
print("Solução: \n", manhatan_sol, '\n')

start = time.time()
bfsSol = bfs(estado_inicial)
end = time.time()
tempo_exec = (end - start) * 1000
print("BFS. Tamanho: ", len(bfsSol), ", Tempo: {:.4f} ms".format(tempo_exec))
print("Solução: \n", bfsSol, '\n')

start = time.time()
dfsSol = dfs(estado_inicial)
end = time.time()
tempo_exec = (end - start) * 1000
print("DFS. Tamanho: ", len(dfsSol), ", Tempo: {:.4f} ms".format(tempo_exec))
#print("Solução: \n", dfsSol, '\n')

start = time.time()
n_swap_sol = astar_n_swap(estado_inicial)
end = time.time()
tempo_exec = (end - start) * 1000
print("A* com nova heurística N-Swap. Tamanho: ", len(n_swap_sol), ", Tempo: {:.4f} ms".format(tempo_exec))
print("Solução: \n", n_swap_sol, '\n')