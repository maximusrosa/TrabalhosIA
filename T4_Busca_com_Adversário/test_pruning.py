    ...
    def test_pruning(self):
        ...
        for non_terminal in ['A', 'B', 'C', 'D']:
            with self.subTest(f"Non-terminal {non_terminal}"):
                ...
                self.assertEqual(calls[non_terminal], 0,
                                 'Erro: a funcao de utilidade nao deve ser chamada nos nao-terminais')
        ...
        for non_pruned in ['E', 'F', 'G', 'H', 'K', 'L', 'M']:
            with self.subTest(f"Non-pruned terminal {non_pruned}"):
                # verifica se a funcao minimax nao chamou a funcao de avaliacao em nenhum estado terminal
                self.assertEqual(calls[non_pruned], 1,
                                 'Erro: a funcao de utilidade deve ser chamada 1 vez para os nos q nao sao podados')
        ...
        for pruned in ['I', 'J']:
            with self.subTest(f"Pruned {non_terminal}"):
                ...
                self.assertEqual(calls[pruned], 0,
                                 'Erro: a funcao de utilidade nao deve ser chamada para os nos q sao podados')
